...
WORKDIR /src
COPY ["day 1 assignment.csproj", "."]
RUN dotnet restore "./day 1 assignment.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./day 1 assignment.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# This stage is used to publish the service project to be copied to the final stage
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./day 1 assignment.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=true

# This stage is used in production or when running from VS in regular mode (Default when not using the Debug configuration)
FROM mcr.microsoft.com/dotnet/runtime:10.0-nanoserver-ltsc2022 AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["day 1 assignment.exe"]